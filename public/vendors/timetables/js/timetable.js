					...
					timetableString += '<div class="timetable-column-content">';
						...
						for (var t = 0; t < timetables.length; t++) {
							if (timetables[t].start_time && timetables[t].end_time) {
								...
								if (timetables[t].agendaFileName == "storage/Meetings/0") {
						        agenda = "none";
						    } else {
						        agenda = "inline";
						    }

								if (timetables[t].attachmentsFileName == "storage/Meetings/0") {
										attachments = "none";
								} else {
										attachments = "inline";
								}
								...
								if (timetables[t].proceedingFileName == "storage/Meetings/0") {
						        proceeding = "none";
						    } else {
						        proceeding = "inline";
						    }

								timetables[t].members = timetables[t].members.replace(/\[/g, "");
								timetables[t].members = timetables[t].members.replace(/\]/g, "");
								timetables[t].members = timetables[t].members.replace(/\"/g, "");
								timetables[t].members = timetables[t].members.replace(/\,/g, " - ");
								timetableString += 	'<div class="timetable-item">'
														...
														+ '<div id="timetable-popup-' + timetables[t].id + '" class="timetable-popup zoom-anim-dialog mfp-hide">'
															...
															+ '<i style="font-size: 20px;float:right;color: #3BAFDA;" class="ft-edit"></i>'
															+ '<a href="/meetings/deleteMeeting/'+ timetables[t].meetingId +'"><i style="font-size: 20px;float:right;color: #DA4453;" class="ft-x-square danger"></i></a>'
																+ timetable_image
																...
																+ '<br><br><div style="text-align: left;">' + 'Meeting Memebers: ' + timetables[t].members+ '</div>'
																+ '<br><br><br><a target="_blank" href="' + timetables[t].agendaFileName+ '"><button style="display: ' + agenda + ';" type="button" class="btn btn-info round btn-min-width mr-1 mb-1">Agenda File</button></a>'
																+ '<a target="_blank" href="' + timetables[t].attachmentsFileName+ '"><button style="display:  ' + attachments + ';" type="button" class="btn btn-info round btn-min-width mr-1 mb-1">Attachments</button></a>'
																+ '<a target="_blank" href="' + timetables[t].proceedingFileName+ '"><button style="display:  ' + proceeding + ';" type="button" class="btn btn-info round btn-min-width mr-1 mb-1">Proceeding File</button></a>'
															+ '</div>'
														+ '</div>'
													+ '</div>';
							}
